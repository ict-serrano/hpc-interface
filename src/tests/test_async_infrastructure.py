...
import hpc.api.services.async_infrastructure as infrastructure
from hpc.api.openapi.models.infrastructure import Infrastructure
import hpc.api.utils.async_persistence as persistence


@pytest.mark.asyncio
async def test_infrastructure_creation(assh_infrastructures):
...
@pytest.mark.asyncio
async def test_infrastructure_retrieval(assh_infrastructures):
    ssh_infrastructures = await assh_infrastructures
    data = ssh_infrastructures[0]
