...
RESULT = f"serrano/data/Output_Data/KMean/KMean_cluster.csv"
...
async def submit_slurm_job_kmeans():
...
async def hpc_transfer_to_s3():
    ...
    data = {
        "endpoint": s3.get("endpoint"),
        "bucket": s3.get("bucket"),
        "object": f"result_{ServiceName.KMEAN}_{DATA.name}",
        "region": s3.get("region"),
        "access_key": s3.get("access_key"),
        "secret_key": s3.get("secret_key"),
        "src": RESULT,
...
async def check_s3_result():
    ...
    client = s3_client(config)
    bucket = config.get("bucket")
    obj = f"result_{ServiceName.KMEAN}_{DATA.name}"
